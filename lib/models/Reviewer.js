...
module.exports = class Reviewer {
  ...
  static async update(id, newAttributes) {
    ...
    //   alternative deconstructing
    //   const updatedAttributes = { ...oldAttributes, ...newAttributes };
  ...
  static async delete(id) {
    // reviews related to the reviewers must be deleted first
    // await pool.query('DELETE FROM reviews WHERE reviewer_id=$1;', [id]);
    const { rows } = await pool.query(
